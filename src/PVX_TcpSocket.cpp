...
//		}
		...
		int TcpSocket::SendFragmented(const std::vector<unsigned char> & data, int FragmentSize) {
			size_t Offset = 0;
			size_t sz = data.size();
			int SendSize = FragmentSize;
		...
		void TcpServer_Old::Serve(std::function<void(TcpSocket)> Event) {
			Running = 1;
